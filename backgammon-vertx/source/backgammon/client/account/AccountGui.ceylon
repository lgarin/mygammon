...
shared class AccountGui(Document document) extends BoardGui(document) {
	...
	function buildTransactionData([PlayerTransaction*] transactions) {
	...
	shared void showTransactions([PlayerTransaction*] transactions) {
		value transactionData = buildTransactionData(transactions);
		dynamic {
			jQuery("#transaction-table tbody").loadTemplate(jQuery("#transaction-row-template"), JSON.parse(transactionData.string));
