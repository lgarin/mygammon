...
shared final class JoinedTableMessage(shared actual PlayerId playerId, shared actual TableId tableId, shared actual Boolean success = true) satisfies OutboundTableMessage & RoomResponseMessage {
...
shared final class TableStateResponseMessage(shared actual PlayerId playerId, shared actual TableId tableId, shared Integer queueSize, shared Boolean joined, shared MatchState? match, shared actual Boolean success) satisfies OutboundTableMessage & RoomResponseMessage {
	toJson() => toExtendedJson({"queueSize" -> queueSize, "joined" -> joined, "match" -> match?.toJson(), "success" -> success});
...
shared TableStateResponseMessage parseTableStateResponseMessage(Object json) {
	return TableStateResponseMessage(parsePlayerId(json.getString("playerId")), parseTableId(json.getObject("tableId")), json.getInteger("queueSize"), json.getBoolean("joined"), parseNullableMatchState(json.getObjectOrNull("match")), json.getBoolean("success"));
}
